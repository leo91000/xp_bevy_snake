...
pub struct SteppingUiState {
    visibility: bool,
}

impl SteppingUiState {
...
impl Plugin for SteppingEguiPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.insert_resource(SteppingUiState::default());

        app.add_systems(
            SteppingSchedule,
            (draw_window, handle_input, toggle_ui_stepping),
        );
    }
}
...
fn toggle_ui_stepping(
    keyboard_input: Res<ButtonInput<KeyCode>>,
    mut ui_state: ResMut<SteppingUiState>,
) {
    if keyboard_input.just_pressed(KeyCode::F11) {
        ui_state.toggle_visibility();
    }
}
